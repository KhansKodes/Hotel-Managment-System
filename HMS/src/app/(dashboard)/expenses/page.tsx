...
import { Receipt, MoreHorizontal, Edit, Trash2, Download, Search, X } from 'lucide-react';
import { ExpenseSheet } from '@/components/expenses/expense-sheet';
import { format, getMonth, getYear, setMonth, setYear } from 'date-fns';
import { DEFAULT_EXPENSE_CATEGORIES, type Expense } from '@/lib/types';

export default function ExpensesPage() {
  const { monthlyExpenses, loading, deleteExpense, currentDate, setCurrentDate } = useHMS();
  ...
  const handleExport = () => {
    exportToCSV(
      ...
      `expenses-${format(currentDate, 'yyyy-MM')}`
    );
  };
  ...
  return (
    <>
      ...
      <main className="p-3 sm:p-6">
        <PageHeader
          title="Expenses"
          description={`${format(currentDate, 'MMM yyyy')} • Total: ${formatCurrency(totalExpenses)}`}
          action={{ label: 'Add Expense', onClick: handleAdd }}
        >
          ...
          <Card>
            <CardContent className="p-0">
              <EmptyState
                ...
                description="Track your restaurant's operating expenses like rent, utilities, and maintenance."
                action={{ label: 'Add Expense', onClick: handleAdd }}
              />
            </CardContent>
          </Card>
                    ...
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex items-start gap-2.5">
                        ...
                        <div className="min-w-0">
                          <h3 className="font-medium text-sm truncate">{expense.title}</h3>
                    ...
                    <TableBody>
                      {filteredExpenses.map((expense) => (
                        <TableRow key={expense.id}>
                          ...
                          <TableCell className="capitalize hidden md:table-cell text-sm">
                            {expense.paymentMethod || '—'}
                          </TableCell>
                          <TableCell className="text-right font-medium text-destructive text-sm">
                            -{formatCurrency(expense.amount)}
                          </TableCell>
                          <TableCell className="hidden md:table-cell text-sm">{formatDate(expense.expenseDate)}</TableCell>
                          <TableCell>
                            <DropdownMenu>
                              <DropdownMenuTrigger asChild>
